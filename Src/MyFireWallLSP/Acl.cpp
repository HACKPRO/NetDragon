...
CHAR			g_szPhoenixFW[MAX_PATH];			  // 记录主程序路径
#pragma bss_seg()


extern CHAR	g_szCurrentApp[MAX_PATH];
...
{
	// 如果是主模块访问网络，放行
	if(strcmp(g_szCurrentApp, g_szPhoenixFW) == 0)
	{
		return PF_PASS;
	}
...
int CAcl::FindRule(CHAR *szAppName, int nStart)
{
	// 从指定位置开始查找，返回规则的索引
	int nIndex;
	for(nIndex = nStart; nIndex < (int)g_RuleCount; nIndex++)
	{
		if(strcmp(szAppName, g_Rule[nIndex].szApplication) == 0)
			break;
	}
	return nIndex;
}
...
BOOL CAcl::QueryAccess()
{
	...
	for(int i=0; i<MAX_QUERY_SESSION; i++)
	{
		if(!g_QuerySession[i].bUsed) // 找到一个没有使用的QuerySession，发出询问
		{
			...
			strcpy(g_QuerySession[i].szPathName, g_szCurrentApp);
...
int CAcl::CreateSession(SOCKET s, int nProtocol)
{
	...
	strcpy(m_pSession[m_nSessionCount].szPathName, g_szCurrentApp);
	m_nSessionCount++;
...
int __stdcall PLSPIoControl(LSP_IO_CONTROL *pIoControl, int nType)
{
	...
	case IO_CONTROL_SET_PHOENIX_INSTANCE:	// 设置主模块信息
		{
			...
			strcpy(g_szPhoenixFW, pIoControl->szPath);
		}
		break;
	...
	case IO_CONTROL_GET_QUERY_SESSION:		// 获取发出询问的会话
		{	
			strcpy(pIoControl->szPath, g_QuerySession[pIoControl->nSessionIndex].szPathName);
		}
		break;
	}
	return 0;

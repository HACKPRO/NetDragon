...
BEGIN_MESSAGE_MAP(CRuleDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_CBN_SELCHANGE(IDC_DIRECTION, &CRuleDlg::OnCbnSelchangeDirection)
...
BOOL CRuleDlg::OnInitDialog() 
{
	CDialog::OnInitDialog();
		...
		// 初始化窗口中的各个控件资源
	// 采取的动作
	m_ComboAction.InsertString(0, "放行");
	m_ComboAction.InsertString(1, "拒绝");

	// 进出方向
	m_ComboDirection.InsertString(0, "进");
	m_ComboDirection.InsertString(1, "出");
	m_ComboDirection.InsertString(2, "双向");

	// 服务类型
	m_ComboType.InsertString(0, "全部");
	m_ComboType.InsertString(1, "TCP");
	m_ComboType.InsertString(2, "UDP");
	m_ComboType.InsertString(3, "FTP");
	m_ComboType.InsertString(4, "TELNET");
	m_ComboType.InsertString(5, "HTTP");
	m_ComboType.InsertString(6, "NNTP");
	m_ComboType.InsertString(7, "POP3");
	m_ComboType.InsertString(8, "SMTP");
	...
	{
		CString s;
		s.Format("%s 要访问网络，放行吗？", m_sPathName); 
		m_RuleTitle.SetWindowText(s);
		m_ComboApp.SetWindowText(m_sPathName);
	}
	else
	{
		if(_tcscmp(m_RuleItem.szApplication, "") != 0)
		{
	...
	// 设置端口号编辑框中的内容
	CString s;
	s.Format("%d", m_RuleItem.usServicePort);
...
{	
	...
	{
		MessageBox("端口值无效，有效范围为 0 - 65535，请重新输入。");
		m_EditPort.SetFocus();
		return;
	}
...
void CRuleDlg::OnCbnSelchangeDirection()
{
	// TODO: 在此添加控件通知处理程序代码
}

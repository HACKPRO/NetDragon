...
CPIOControl::CPIOControl()
{
	...
	{
		::MessageBox(NULL, "警告:装载LSP出错,请检查系统,某些功能将无法使用!", "error", 0);
		exit(0);
	}
}
...
void CPIOControl::SetPhoenixInstance(HWND hWnd, CHAR *pszPathName)
{
	// 设置实例句柄
	m_IoControl.hPhoenixWnd = hWnd;
	strcpy(m_IoControl.szPath, pszPathName);
...
LPCSTR CPIOControl::GetQueryApp(int nIndex)
{
	// 获取查询的应用程序
	m_IoControl.nSessionIndex = nIndex;

...
// 	m_rules.InsertColumn(0, "应用程序", LVCFMT_LEFT, sizeof("应用程序")*8, 0);
// 	m_rules.InsertColumn(1, "动作", LVCFMT_LEFT, sizeof( "动作")*8, 1);
// 	m_rules.InsertColumn(2, "类型/端口", LVCFMT_LEFT, sizeof("类型/端口")*8, 2);
// 	m_rules.InsertColumn(3, "应用程序路径", LVCFMT_LEFT, sizeof("应用程序路径")*12, 3);
// 	m_rules.InsertColumn(4, "说明", LVCFMT_LEFT, sizeof("说明")*12, 4);

	m_rules.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
	m_rules.InsertColumn(0, "应用程序"		, LVCFMT_LEFT	,  120	, 0);
	m_rules.InsertColumn(1, "动作"			, LVCFMT_LEFT	,  120  , 1);
	m_rules.InsertColumn(2, "类型/端口"		, LVCFMT_LEFT	,  100  , 2);
	m_rules.InsertColumn(3, "应用程序路径"	, LVCFMT_LEFT	,  120  , 3);
	m_rules.InsertColumn(4, "说明"			, LVCFMT_LEFT	,  120  , 4);

	// 更新列表，即向列表中添加规则
	UpdateList();

	// 无效删除和编辑按钮
	GetDlgItem(IDC_DELE)->EnableWindow(FALSE);
...
void CRulePage::EditARule(RULE_ITEM *pItem, int nEditIndex)
{
	...
	if(nEditIndex >= 0)
		...
		m_rules.InsertItem(nIndex, "", 0);

	// 构建新项的文本
	CString sAction, sServType;

	sAction = (pItem->ucAction == 0) ? "放行" : "拒绝";

	switch(pItem->ucServiceType)
	{	
	case RULE_SERVICE_TYPE_ALL:
		sServType.Format("所有/%d", pItem->usServicePort);
		break;
	case RULE_SERVICE_TYPE_TCP:
		sServType.Format("TCP/%d", pItem->usServicePort);
		break;
	case RULE_SERVICE_TYPE_UDP:
		sServType.Format("UDP/%d", pItem->usServicePort);
		break;
	case RULE_SERVICE_TYPE_FTP:
		sServType.Format("FTP/%d", pItem->usServicePort);
		break;
	case RULE_SERVICE_TYPE_TELNET:
		sServType.Format("TELNET/%d", pItem->usServicePort);
		break;
	case RULE_SERVICE_TYPE_HTTP:
		sServType.Format("HTTP/%d", pItem->usServicePort);
		break;
	case RULE_SERVICE_TYPE_NNTP:
		sServType.Format("NNTP/%d", pItem->usServicePort);
		break;
	case RULE_SERVICE_TYPE_POP3:
		sServType.Format("POP3/%d", pItem->usServicePort);
		break;
	}
...
int CRulePage::InitAddRule(LPCSTR szQueryApp)
{
	if(g_RuleFile.m_header.ulLspRuleCount > MAX_RULE_COUNT)
	{
		AfxMessageBox(" 超过规则的最大数目，不能够再添加");
		return -1;
	}
	...
	_tcscpy(tmpRule.sDemo, "");
	_tcscpy(tmpRule.szApplication, "");
	...
	if(CRuleDlg::m_bAppQuery)
		((CNetDragonDlg*)theApp.m_pMainWnd)->m_sheet.SetActivePage(this);
...
BOOL CRulePage::AddQueryRule(LPCSTR pszQueryApp) // 静态函数
{
	int nRet = ((CNetDragonDlg*)theApp.m_pMainWnd)->m_RulePage.InitAddRule(pszQueryApp);
	if( nRet == -1 )
		return FALSE;
	...
	// 无效主对话框的应用按钮
	((CNetDragonDlg*)theApp.m_pMainWnd)->GetDlgItem(IDC_APPLY)->EnableWindow(FALSE);
...
void CRulePage::OnItemchangedRules(NMHDR* pNMHDR, LRESULT* pResult) // 用户改变所选项
{
	...
	if((m_nListIndex = pNMList->iItem) != -1)
	{
		GetDlgItem(IDC_DELE)->EnableWindow(TRUE);
